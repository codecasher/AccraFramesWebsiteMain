...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const qty = getSelectedPiecesCount();

  const retailPrice = parseFloat(selected.dataset.price || 0);
  const wholesalePrice = parseFloat(
    selected.dataset.wholesale || retailPrice
  );

  // Wholesale applies at 5 items or more
  return qty >= 5 ? wholesalePrice : retailPrice;
}
...
colorCheckboxes.forEach(cb => {
  cb.addEventListener('change', function() {
    ...
    if (this.checked) {
      ...
      // Update color code element for WhatsApp/cart
      const colorCodeEl = qs('#Color-code');
      if (colorCodeEl) colorCodeEl.textContent = `Colour-code: ${code}`;
    }
  });
});
  ...
  addButtons.forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      const colorCode =
       qs('#Color-code')?.textContent.replace('Colour-code:', '').trim() || '';


      let items = readCart();
      const existing = items.find(i => i.key === key);
      if (existing) existing.qty += pieces;
      else items.push({ key, name, size, color, colorCode, price: pricePerPiece, img, qty: pieces });
...
if (buyNowBtn) {
  buyNowBtn.addEventListener('click', () => {
    ...
    if (items.length === 0) {
      ...
      if (frameColorCode) message += `Colour Code: ${frameColorCode}%0A`; // <-- Include code
      message += `Quantity: ${quantity} pcs%0A`;
      message += `Total Price: ${totalPrice}%0A%0A`;
    } else {
      // Cart items already include colorCode
      let grandTotal = 0;
      items.forEach(it => {
        ...
        if (it.colorCode) message += `Colour Code: ${it.colorCode}%0A`; // <-- Already included
    ...
    const phoneNumber = "254714821110";
  ...
  if (checkoutBtn) {
    checkoutBtn.addEventListener('click', e => {
      ...
      let message = "🛒 *Checkout Order from AccraFrames!*%0A%0A";
      let total = 0;
      items.forEach((item, i) => {
        ...
        if (item.colorCode)
          message += ` - Colour Code: ${item.colorCode}%0A`;
      ...
      message += `%0AThank you for shopping with AccraFrames! 🙏`;
